...
public class MemberDerWocheCommand extends CommandBase implements IClientCommand {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
        new Thread(() -> {
            ...
            try {
                 list = SheetUtils.getValueRange("\u00dcbersicht", "C44:K44").getValues();
            } catch (IOException e) {
            ...
            player.sendMessage(new TextComponentString(TextFormatting.GRAY + "   " + list.get(0).get(4).toString()));
            player.sendMessage(new TextComponentString(TextFormatting.GOLD + " " + TextFormatting.BOLD + "RolePlay"));
            player.sendMessage(new TextComponentString(TextFormatting.GRAY + "   " + list.get(0).get(7).toString()));
            player.sendMessage(new TextComponentString(TextFormatting.GOLD + " " + TextFormatting.BOLD + "Leitungen"));
